...
- **App & action launcher** — Spotlight-style search to launch apps and run actions.
  ...
  - **Ask AI** — send the query to a model and read the streamed answer in a small floating window, with follow-ups. Each window can drop out of always-on-top (pin button in its titlebar, or Window → Float on Top, `⌃`+`⌘`+`T`) and become an ordinary window, at which point the app takes a Dock icon and a menu bar for as long as one is open. Endpoint, model, API format (`/v1/messages`, `/v1/responses`, or `/v1/chat/completions`), web search, and API key are set in Settings → AI; the key is kept in the keychain.
